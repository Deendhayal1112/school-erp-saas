...
class SuccessResponse(BaseModel, Generic[T]):
    ...
    message: str = Field(
        "Operation successful", description="A short, readable status message."
    )
    data: T | None = Field(
        None, description="The primary return payload data (optional)."
    )
...
class ValidationErrorDetail(BaseModel):
    ...
    loc: list[str] = Field(
        ..., description="Field path location of the validation error."
    )
...
class ValidationErrorResponse(BaseModel):
    ...
    success: bool = Field(False, description="Indicates operation failed.")
    error: str = Field(
        "ValidationError", description="Validation error classification code."
    )
    message: str = Field(
        "Input validation checks failed", description="Summary description."
    )

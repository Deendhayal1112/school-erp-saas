...
class PaginationMetadata(BaseModel):
    ...
    total_records: int = Field(
        ..., description="Total count of database records matching the filter criteria."
    )
    page: int = Field(..., description="Active requested page index (1-indexed).")
    page_size: int = Field(
        ..., description="Count limit of records returned per query page."
    )
    total_pages: int = Field(
        ..., description="Calculated count of available result pages."
    )
    next: str | None = Field(
        None,
        description="Request URI path reference to pull the subsequent page payload.",
    )
    previous: str | None = Field(
        None,
        description="Request URI path reference to pull the preceding page payload.",
    )
...
class PaginatedResponse(BaseModel, Generic[T]):
    ...
    success: bool = Field(
        True, description="Indicates operation completed successfully."
    )
    message: str = Field(
        "Query completed successfully", description="Status message description."
    )
    pagination: PaginationMetadata = Field(
        ..., description="Query pagination offsets and total count references."
    )
